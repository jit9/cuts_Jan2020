...
#+END_SRC
Most of the scripts in this repo contain only definition of
different ~Routine~ (more on this later). These routines can be added
to a pipeline that executes on a list of TODs. A simple pipeline is
given in ~test.py~ which consists of only two routines. The first routine
loads the TOD and the second routine compute a few features of the loaded
TOD. To run it
#+BEGIN_SRC sh
python test.py
#+END_SRC
